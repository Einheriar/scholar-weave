...
- **窄屏抽屉右上角是固定尺寸的「历史记录 → 新文章」变形按钮。** 默认态只用极淡品牌底色与无方向环绕柔影提示可按；指针移入后交叉淡入「新文章」，操作态配色与当前项目完全一致（`brand-soft` 背景、`brand-ring` 边线、`brand` 文字），按钮本体在平面命中区内做明显但受控的 3D 倾斜（rotateX 最大 ±8°、rotateY 最大 ±12°）与指针高光。倾斜只能写在内层 `.t-drawer-new-button`，不要覆盖其 `absolute` 定位；外层 `.t-drawer-title-control` 保持不变形，否则命中区会漂。抽屉滑入会穿过静止指针，因此外层在前 280ms 禁用 pointer events，避免未移动鼠标就误进 hover。打开时初始焦点放在 `role="dialog"` 的面板本身，而不是透明叠放的「新文章」按钮；键盘 Tab 聚焦按钮时再显出操作态。`prefers-reduced-motion` 下禁用倾斜与高光。
