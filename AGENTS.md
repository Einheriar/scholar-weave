...
17. **提示词的指令语言与输出语言分离：内部指令统一英文，解释永远中文，replacement 跟文档语言。** `buildSystemPrompt` / `buildChatMessages` / `buildChangeSetMessages` 生成的 system 消息与固定 user 信封都用英文，以提高模型遵从性；用户原话、文档内容、自定义要求和保留术语保持原样，不做翻译。用户场景只有中文文档 / 英文文档两种，`explanationLanguage` 固定为 Chinese，`language` 只决定 replacement 写成中文还是英文。`page.tsx` 里 `language: "en"` 是三处写死的，不要把它当 bug 改掉。
