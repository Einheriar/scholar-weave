...
字体统一从 `src/app/fonts/` 通过 `next/font/local` 加载，包括 MiSans、Inter 和 Geist Mono。不要重新引入 `next/font/google`，否则生产构建会依赖 Google Fonts 网络下载；新增字体时保留来源和许可证。
...
15. **修改集中的纯前置／后置新增只改变预览展示。** 当 replacement 逐字保留整个 original，仅在一端增加内容时，显示“新增 · 在以下原文之前／之后”，原文不加删除线，单独突出新增内容；前后两端均匹配而位置有歧义、锚点内部插入或混合改写仍显示完整差异。不裁剪空白、不改变原 edit，定位、接受与撤销继续走既有替换协议。
...
- **窄屏抽屉右上角是固定尺寸的「历史记录 → 新文章」变形按钮。** 默认态只用极淡品牌底色与无方向环绕柔影提示可按；指针移入后交叉淡入「新文章」，操作态配色与当前项目完全一致（`brand-soft` 背景、`brand-ring` 边线、`brand` 文字），按钮本体在平面命中区内做明显但受控的 3D 倾斜（rotateX 最大 ±8°、rotateY 最大 ±12°）。视觉光效分两层：彩虹细边和青绿／紫色外辉光在 hover 期间保持固定，不随指针换色或移动；只有 `20×12px`、低透明度的中性灰白反光跟随指针。倾斜只能写在内层 `.t-drawer-new-button`，不要覆盖其 `absolute` 定位；外层 `.t-drawer-title-control` 保持不变形，否则命中区会漂。抽屉滑入会穿过静止指针，因此外层在前 280ms 禁用 pointer events，避免未移动鼠标就误进 hover。打开时初始焦点放在 `role="dialog"` 的面板本身，而不是透明叠放的「新文章」按钮；键盘 Tab 聚焦按钮时再显出操作态。`prefers-reduced-motion` 下禁用倾斜与光效过渡。
...
- **重复点击当前项目只收起历史抽屉，不重新载入项目快照。** `projects` 中的内容可能比编辑器落后一个 500ms 防抖窗口；按当前 id 再次 `setDoc(proj.doc)` 会覆盖尚未落库的正文、标题和其他现场，并取消待执行保存。
- **内容变更统一通过 `markProjectDirty()` 标记待保存。** 它同步推进 `saveVersionRef`，覆盖正文、标题、建议与聊天节点；每次保存开始和真正切换项目也推进版本。异步保存完成后，只有版本与当前项目 id 都仍匹配才能置为 `saved`，否则会取消后续编辑的防抖保存。首次建档和项目切换时也必须同步更新 `latestRef` 的项目 id，不能只等待 React effect。相关回归测试在 `tests/project-autosave.test.tsx`。
- **右上角软盘按钮可立即保存最新现场。** 手动保存复用 `persistProjectNow`，数据立即写入，但保存反馈与重复点击锁至少保持 500ms；实际写入更慢时等待写入完成。光晕每 500ms 转一圈，手动点击重新开始动画，`prefers-reduced-motion` 下停止旋转。手动保存反馈期间暂停防抖；期间发生的新编辑仍推进保存版本，反馈结束后继续自动保存。明确点击保存允许未建档文章立即建档。
- **删除当前项目不能留下仍会自动保存的正文。** 删除期间锁住操作，用同步删除标记阻止新自动保存，停止当前项目防抖并使旧保存回调失效；等待其在途自动保存结束后再删数据库记录。成功后同步把正文、建议、节点、项目 id 与保存状态重置到未建档空白现场；失败则保留现场并恢复被暂停的保存。删除其他项目不能中断当前项目的保存。
- **改标题也必须调用 `markProjectDirty()`**：它是一次内容编辑，不置 saving 就不触发防抖保存，标题既不落库，也不算活动。它不走 `handleDocChange` 是为了跳过锚点校验（标题不参与 block 定位）。
- **项目标题派生**：`deriveProjectTitle` 优先取文档手动标题，否则从第一个非空段落本地生成；空格分词文本取前 12 个词，连续文本（如中文）取前 24 个字符，并以 96 个字符作为异常长词硬上限；兜底「未命名文章」。自动标题会在首次建档保存时写入 `doc.title`，因此标题框与历史条目保持联动，不经过 LLM。

### 宽屏顶栏与当前项目清空

- `xl` 宽屏顶栏与正文区使用同一套三列几何：`240px 历史栏 | 正文列（最小 750px） | 360px 审阅栏`，列间距均为 `24px`。正文列在顶栏里拆成 `1fr | 操作组 | 1fr`，审阅模式、开始审阅、复制全文、清空项目这一组因此始终以**正文列中心**为基准；标题自适应占满它左侧的全部空间并留 12px 间距。不要退回固定标题宽度或“跟在标题后面”的普通 flex 排列。`xl` 以下标题是 `flex-1/min-w-0` 弹性项，应优先压缩标题来保证操作组不越出顶栏右边界，实在放不下才自然换行。
- **最外层 `main` 的响应式最小宽度必须包含自身左右 padding。** `lg` 双栏内容至少需要 `750 + 24 + 360 = 1134px`，加两侧各 24px 后 `main` 为 `1182px`；`xl` 再加 240px 历史栏与 24px 间距，`main` 为 `1446px`。否则下方栏位会从父容器右侧溢出，而审阅状态／错误／总结横幅仍只铺父容器宽度，右边缘就无法与审阅侧栏对齐。对应 E2E 会在 1180px 与 1600px 两种宽度测量右边界。
- 顶栏「N 条待处理」与审阅侧栏计数来自同一数据。`lg` 及以上侧栏同时可见时隐藏顶栏副本；较窄视口侧栏落到正文下方时保留顶栏计数，方便用户不用滚动也能看到状态。
- 顶栏「清空项目」只重置**当前项目**的标题、正文、审阅建议和聊天节点，保留项目 id，并立即落库；其他历史项目不受影响。尚未建档的现场只重置、不为纯空白内容额外创建历史条目。该操作必须确认、请求期间禁用，并与设置里的「清空数据」（删除全部项目）严格区分。

### 聊天节点（ChatNode）与锚点

- **聊天已有明确方案后，要求直接生成修改应优先返回修改集。** 回复协议与行为规则必须一致：`answer_with_review` 只用于用户明确希望先记录意见、不生成具体改文的场景，不能仅因讨论形成共识就重复生成意见；单纯表示同意仍不视为修改请求。新增文字可通过保留原文的 replacement 表达，正文仍须经预览确认。

- 原文锚点失效时禁用“重新生成回复”，悬停提示“原文已变更，请重新选择正文后提问。”；发送入口也要按最新正文复核，阻止失效节点的重新生成及其失败重试。输入框仍可继续讨论旧片段，且仅允许纯文字回复。

- 图片入口只保留输入框粘贴／拖入，不显示独立的添加图片按钮或空附件占位。每条消息最多 4 张，超限时在输入框内提示，已添加图片保留，删除后可继续添加。
...
- **聊天→正文的反向定位**：时间线每行左侧的节点身份竖条与聊天头部“当前上下文”标签都可回到正文锚点。竖条仍是次要入口，视觉保持 3px、命中区只温和扩大到 `12×24px`；范围锚恢复真实文本选区，review 锚复用建议高亮，block/document 只落光标，避免被 `onSelectionUpdate` 误判成新的 range 节点。滚动位置按 sticky 聊天框上方的实际可读区域计算，不可唯一定位的 stale 锚禁用入口并明确提示，绝不猜位置。
...
- **range 节点的定位证据与模型上下文严格分离**：节点身份仍只按 `selectedText` 判断；但新建节点会把编辑器生成的段内起止位置、前后文和段落快照存入 `ChatNode.rangeLocator`，用于重复文本消歧及正文局部编辑后的重新定位。编辑器打开期间用 ProseMirror transaction mapping 持续更新该证据，刷新后再由快照与文本上下文恢复。该字段不得放进 `ChatContext` 或 `/api/chat` 请求，模型只看到选中文字和业务上下文。旧项目没有该字段时继续走逐字定位兜底。
- **跨段人工选区自动附带全文，仍保留 range 节点。** `rangeLocator.blockIds` 可选字段记录连续段落 ID，快照按单个空格连接，与选区文本一致；定位后拆回各段范围供高亮、返回选区与 transaction mapping 使用。段落删除、插入或重排导致不连续时拒绝定位，不能只校验首段或把多段选文塞进首段搜索。发送、重试、重新生成与全文开关显示共用同一判定；人工重新选择必须在选区回调中立即校验、更新并保存已有同名节点的本地定位证据，不能等发送才刷新，否则旧节点仍显示 stale 并禁用操作；重试不重绑。多段范围无法表达为现有单段 ReviewScope，转意见时明确提示直接生成修改集，不静默升级为全文意见。
- **规则 11（无锚点默认禁发）**：无选区、无选中建议、没有正在查看的历史节点且未开启全文背景时禁发；查看历史局部节点时允许直接沿该节点追问，不要求重新选区。显式开启「附带全文背景」或进入固定全文节点后可以基于全文提问。E2E 新建局部聊天仍须先 `selectTextInEditor`。
- **规则 12（stale 锚）**：锚点定位失败时标记消失，但仍可讨论旧片段与存活的当前段落。发送的是最新正文片段与本节点旧消息，绝不把定位用的 `rangeLocator.blockText` 当历史正文发给模型；局部锚点段落已删除时 `blocks` 为空，不得自动回退全文。每次发送／重试／重新生成重新计算 `anchorStale`，提示词区分旧引文与当前正文，服务端和前端均把失效锚回复限制为纯 `answer`，禁止可执行修改和新候选意见，开启全文背景也不能绕过。失效节点的旧修改集预览禁用，已有讨论保留可读。
- **规则 24（上下文边界 = 节点边界）**：发给模型的 history 只有本节点轮次；openReviews 只带锚点所在段落的 open 建议。
- **聊天图片属于 user turn，不能混入正文或锚点。** `ChatTurn.images` 为可选字段，旧项目无需迁移；附件随项目存入 IndexedDB，只在发送时传给配置的模型。重试和重新生成从原 user turn 恢复图片，历史上下文保留图片但仍按本节点最近轮次裁剪。上传图片不自动授权全文；原有选区／节点／显式全文背景规则继续生效。图片必须经过类型和体积校验，模型接口使用多模态 `image_url` 内容块，不能把 Base64 拼进文本提示词。
- **图片输入能力按模型预设手动配置。** `LLMPreset.imageInputEnabled` 缺省为开启，旧配置保持兼容；关闭时禁止添加图片，已有附件仍可移除、历史图片仍可查看。含本轮或实际发送历史图片的请求须明确拦截并保留草稿，重试／重新生成也必须检查；不能静默剥离图片。聊天请求独立传递此开关，服务端在调用模型前复核，不能把它作为供应商参数透传。
...
- **修改集打开期间临时收起聊天，预览与聊天共用底部吸附容器。** `ChangeSetPreview` 位于 `[data-editor-dock]` 内、收起的聊天条上方，列表内部滚动，标题与接受／放弃操作保持可见，面板高度不超过视口的 55%。打开时记录聊天原展开状态，焦点切换使用 `preventScroll`，不要再滚到正文末尾；正文定位以整个 dock 的顶边计算可读区域，不能只测聊天条。接受、放弃或 Escape 关闭后恢复原状态。预览期间主动展开聊天等同放弃本次预览并展开，但聊天轮次中的 changeSet 保留，之后可重开。此状态机与候选审阅意见的两步式转场相互独立。
- **review 节点的聊天修改集要关联来源意见。** `/api/chat` 返回的 ChangeSet 不带 `sourceReviewId`，打开预览时由当前聊天节点的可信 review 锚点补上并固定，不能在接受时再读可能已切换的侧栏选中项。接受时仅当来源是 `open/opinion`、且所选修改命中该意见的段落（全文意见除外）才将其标为 `accepted` 并保存撤销快照；这样旧聊天回复重开预览也能正确处理，撤销时正文和意见一起恢复。
...
- **“查看审阅意见”的上下文转场不能瞬间卸载旧消息。** 主效果是消息抽屉沿底部方向收拢，而不是内容淡没：点击后立刻启动 440ms grid 行高过渡，消息只降到 88% 透明度并随抽屉下移 10px；90ms 时提前定位右侧卡片，以一次 520ms、无阴影的品牌色边框扩散接住视觉重心。输入框和聊天外框始终留在页面上。`data-chat-context-transition` 与 `data-chat-message-collapse` 是 E2E 的稳定观察点。相关 animation/transition 必须纳入 `prefers-reduced-motion`；减少动态效果下直接切换。
- 节点时间线（`NodeTimeline`）：聊天区头部「聊天节点历史」按钮**从聊天区顶部向上滑出的抽屉**（第二层抽拉，不是居中弹窗；标题行 `sticky top-0`），按钮是 **toggle**（再按一次收起，带 `aria-expanded`，开态琥珀底高亮）。第一行始终是固定的「全文」入口：从未聊天时以虚拟 `0 问` 节点呈现（不写入 IndexedDB），首次发送才实体化；清空全文讨论后恢复虚拟入口、不能删除该入口。其余每行一个局部节点，行内删除直接删（规则 13，无确认）。**轨道上的端点 = 用户提问，数量恒等于提问次数**（1 次提问 = 1 个端点，单端点居中 left:50%）；局部行首是**节点身份竖条**（`h-4 w-[3px]`，琥珀）——**不是圆点、不是端点**，2026-09-16 改竖条就是因为圆点会被误认成「多一次提问」；竖条现在也是返回正文锚点的次要入口，但不可扩大成抢眼的主按钮。hover 端点浮出自定义摘要 tooltip（锚点摘要 + 该次提问截断 48 字），**钉在抽屉顶部**（标题栏下）而非底部（底部会盖住节点行）；它是**绝对定位悬浮层**（脱离文档流 + `pointer-events-none`）——放进流内会把行撑开、端点位移、hover 循环闪烁（踩过的坑）。抽屉展开时与聊天区连成一体：聊天区顶部圆角让位（`rounded-b-2xl`）、共享边框无线、抽屉 `max-h-[42vh]` 内部滚动。
- **「附带全文背景」不改变节点锚点。** 在 range/review/block 节点中开启它，只让本轮请求附带发送时的最新全文，聊天历史、头部标签和节点归属仍留在原局部节点；只有没有局部锚点/活动节点时开启，或主动点击固定全文入口，才进入 document 节点。全文节点天然附带全文背景，复合按钮主体显示开启且不可关闭，左侧图钉仍只控制浏览器级默认偏好。
...
- **聊天头部的中间空白也是收起／展开命中区。** 使用无可见内容的弹性按钮 `data-chat-header-toggle` 填满“当前上下文”与右侧箭头之间的空间，点击语义与箭头完全一致；不得把事件挂到整个头部，否则会吞掉左侧节点历史、上下文定位等独立按钮。透明命中区仍须保留可访问名称与键盘焦点环。
...
- 正文纸张右上角常驻一个轻量撤销按钮，悬停／键盘聚焦提示「撤销 / Ctrl+Z」；按钮表示当前文章、当前会话的正文更改历史（包含单条已接受建议），无法撤回已覆盖落库的历史版本，切换文档 ID 时必须重建编辑器以清空跨文章撤销栈。
- 单条建议的接受仍设置 `addToHistory: false`，避免 ProseMirror 独自撤回正文而让卡片继续显示「已接受」；但编辑器另有一层按原生历史深度排序的安全撤销栈，右上角按钮与 `Ctrl+Z` 会通过 `revertSingleEdit` 同时还原正文和建议状态。建议卡片自身的「撤销」也要同步移除该记录。ChangeSet 批量应用仍作为一次普通正文事务进入原生撤销栈。
...
12. **`revision` 只是计数器，但请求结果的版本防串写已经接入。** 当前约定：
    ...
    - 付费请求期间正文、标题、文章切换/新建/删除/排序、样例载入与清空数据会被锁定；取消审阅只让当前请求失效，不允许旧回调随后串写。节点删除／全文节点清空也必须把 `requestLocked` 传到时间线，按钮通过 `aria-disabled`、禁用样式和 Tooltip 明确提示，并在点击处理器中拦截；不要只在父组件静默返回，否则用户会误以为按钮失效。
...
17. **提示词的指令语言与输出语言分离：内部指令统一英文，解释永远中文，replacement 跟文档语言。** `buildSystemPrompt` / `buildChatMessages` / `buildChangeSetMessages` 生成的 system 消息与固定 user 信封都用英文，以提高模型遵从性；用户原话、文档内容、自定义要求和保留术语保持原样，不做翻译。用户场景只有中文文档 / 英文文档两种，`explanationLanguage` 固定为 Chinese，`language` 决定 replacement 和重写备选正文写成中文还是英文，备选的标题与解释仍为中文。`page.tsx` 里 `language: "en"` 是三处写死的，不要把它当 bug 改掉。历史锚失效时提示词只列 `answer` 协议，不附带修改、候选意见或重写指令；全文 review（无 blockId）与全文节点都按全文上下文描述，不能让模型自行猜测开关是手动还是完整选段时自动开启。
...
26. **一个自然段内部正式支持 `\n`。** `DocumentState.blocks[].text` 用换行字符表示 Shift+Enter，`tiptap-convert.ts` 双向映射为 ProseMirror `hardBreak`。所有程序化替换必须走 `textToPMContent()`，不能直接把 replacement 当 HTML/富文本插入；否则会丢换行，且类似 `<b>` 的原文会被解释成标签。读取 PM 段落或选区时统一用 `pmPlainText()`，将每个 `hardBreak` 保留为一个 `\n`，使纯文本偏移与 PM 行内位置一致；不要用会丢换行的 `textContent` 或把换行转为空格。审阅高亮、修改集预览及聊天锚点映射必须共用此规则。
...
- **模型名称联想来自当前连接的 `/models`，仍允许任意手动输入。** `ModelInput` 聚焦后通过 `/api/models` 获取列表，输入时只做本地筛选；地址、密钥或启用的代理变化立即使旧列表失效，取消旧请求并防止迟到响应覆盖。列表仅在设置组件内存中保留，不把密钥作为持久化缓存键；读取列表只使用显式传入的草稿连接配置，不把服务端环境密钥发送到草稿中的任意地址。建议不自动修改图片输入开关或思考档位。浮层使用 portal 避免设置面板裁切，Escape 优先关闭建议，不关闭整个设置。
