...
- `—`       refactor: 全文审阅、上下文聊天与修改集生成的内部 system 指令及固定 user 信封统一改为英文；用户原话与文档内容保持原样，标题、解释和回答仍要求输出中文，replacement 继续跟随文档语言。
- `—`       polish: 将聊天头部“当前上下文”与右侧箭头之间的空白扩展为透明的收起／展开点击区；不增加可见按钮，并保留左侧节点历史、上下文定位和右侧箭头各自的独立交互。
- `—`       feat: 上下文聊天新增「包含全文｜总是」分段控件；默认无选区／建议仍禁发，显式全文模式可创建 document 节点，完整选段自动附带发送时的最新全文且保留 range 锚点。「总是」仅持久化未来默认。提示词明确可见／可修改范围，服务端额外剔除局部锚点段之外的越界 edits。
- `—`       fix: 点击聊天回复的「预览修改」时临时收起 sticky 聊天区并滚动校正，确保修改集可见；接受、放弃或 Escape 后恢复原聊天状态，预览期间主动展开聊天等同放弃，且原回复中的修改集可再次打开。
...
- `—`       fix: 撤销折角在无历史时仍可展开并显示提示，同时保留浅品牌色与白色图标；单条审阅建议接受后纳入右上角按钮与 `Ctrl+Z` 的统一撤销顺序，撤销时同步恢复正文和建议卡状态。
...
- `—` feat: 审阅、上下文对话与意见转修改集统一增加一次结构化输出自动纠错，只处理 JSON／Schema 错误而不自动重试网络或供应商故障；最终失败时提供保留上下文的手动重试，其中聊天重试复用原节点且不重复插入用户消息。
