...
组内按提交先后排列。普通条目的日期取自提交 author date，版本小节使用发布日期。
...
- `b13fc41` 修复自动保存竞态：点击当前项目不再载入旧快照，旧保存完成不会取消新编辑的保存；删除当前项目会等待已有写入结束并阻止后续保存，避免正文重新出现。
- `b13fc41` 历史讨论支持直接追问；失效锚点可讨论旧片段和当前仍存在的段落，但不提供可执行修改。原段落已删除时不自动发送全文。
- `b13fc41` 统一段内换行的文本提取，修复原文高亮、修改预览、选区和聊天锚点的偏移问题。
- `b13fc41` 精简审阅、聊天和修改集提示词，明确 general 意见范围、历史与当前正文的区别、全文权限及重写条件，并对齐实际协议与编辑器能力。
- `f912d3e` 修正便携包的密钥配置提示。

### 文档与验证

- `597c07d`、`ea306cd`、`0ab6fa9`、`8afc1c5` 更新功能演示的动画 WebP、高清源视频与展示尺寸。
- `b13fc41` 增加保存竞态、删除、历史续聊、段内换行和提示词回归覆盖；280 项单元／集成测试通过。
- 版本统一更新为 `0.3.4`，同步锁文件与版本维护说明；生产构建和版本注入测试通过。

## 0.3.2 - 2026-09-18

### 新功能

- `228676e` 修改集预览支持悬停高亮与点击定位正文；按意见接受修改集时持久化安全撤销快照，意见卡可恢复正文或仅恢复处理状态。

### 修复

- `ffa66af` 为聊天选区保存精确范围定位器，并在普通编辑后映射更新，避免相同文本或标点规范化导致选区锚点误判失效。
- `5c3b0e9` 接受修改集后冻结退出动画期间的校验结果，避免正文已成功修改却短暂显示“原文定位失败”。

### 界面优化

- `2faf108` 统一全文与局部聊天节点的身份列、端点和轨道对齐。
- `4345077` 重新设计“重新生成回复”按钮的悬停、按压和生成中反馈，并为按钮与模型回复气泡预留独立间距。

### 文档

- `51a09a3` 完善中英文项目说明、使用流程与功能演示资源，并修正媒体链接。
