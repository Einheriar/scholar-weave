...
- **图片讨论**：直接在聊天输入框粘贴或拖入 PNG、JPEG、WebP 图片，每条消息最多 4 张，发送前可预览和移除。需要配置支持视觉输入的模型；图片随节点历史保存在本浏览器，发送及后续追问时会作为上下文传给所配置的模型。
...
填写提供商的地址和密钥后，点击模型输入框即可获取模型列表，输入名称时会显示匹配建议。支持方向键和回车选择，也可以手动填写列表外的模型；提供商未开放列表接口时，手动输入仍然可用。
