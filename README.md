...
项目根目录下的 [`plan/`](./plan) 专门存放**计划与变更记录**两类文档，不参与构建：
...
- 用户文档草稿与**对话历史**仅保存在**本浏览器**的 IndexedDB，不会上传到本服务之外。
  **草稿只保留最近一份、每次自动保存以覆盖方式写入，不保留历史版本**（所以没有「撤回历史版本」功能，编辑器内的 `Ctrl+Z` 也只能回溯当前会话）；
...
- **界面** — 深浅色主题切换、左侧对话历史、键盘快捷键（`Cmd/Ctrl+Enter` 审阅、`Cmd/Ctrl+Shift+C` 复制全文）、
  屏幕阅读器播报与键盘可达。
