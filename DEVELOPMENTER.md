...
- **每次具体发送什么**（实测请求体）：
  ...
  - 对话历史带**最近 8 条消息的文字与用户图片附件**，**不含文档正文快照**（正文以本次 `blocks` 为准，避免重复上传）。
...
图片附件使用可选的 `ChatTurn.images` 存储，旧项目无需迁移。输入框粘贴与拖拽共用浏览器压缩流程，不显示独立添加按钮；每条消息最多 4 张，超限提示显示在输入框内。当前轮次、重试、重新生成和本节点近期历史均通过 OpenAI 兼容的 `image_url` 内容块发送。图片保存在 IndexedDB，发送时交给当前配置的模型服务，不使用额外图床。所选模型必须支持视觉输入；应用不会自动更换模型。图片作为不可信参考资料，不会成为可执行正文坐标，上传图片也不自动开启全文背景。
